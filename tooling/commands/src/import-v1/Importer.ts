...
class Importer {
  ...
  protected async createSections() {
    ...
    const firstLocale = this.manifest.languages.at(0)
    if (!firstLocale) {
      throw new Error('Course has no locales')
    }

    // Walk down first tree of first locale
    const startDir = path.join(this.importFolder, firstLocale)
  ...
  protected static convertPandocSyntax(content: string) {
    ...
    const [, yaml, source] = matches

    if (!yaml) {
      throw new Error('No YAML header found')
    }
