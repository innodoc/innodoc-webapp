...
import { ContentLink, PageLink, SectionLink } from '.'
...
describe('<ContentLink />', () => {
  it.each([
    ...
    const wrapper = shallow(<ContentLink href={`/${contentType}/foo/bar`} />)
  ...
  it.each([
    ...
    const wrapper = shallow(<ContentLink href={href} />)
  ...
  it('should render with malformed href (development)', () => {
    const wrapper = shallow(<ContentLink href="/foo/bar">Foo</ContentLink>)
