...
export default class Head extends NextHead {
  getCssLinks() {
    ...
    const cssLinksSorted = []
    SEARCH_STRINGS.forEach((searchString) => {
      for (;;) {
        const preloadLink = findAndRemove(cssLinks, searchString, 'preload')
        if (!preloadLink) {
          break
        }
        const stylesheetLink = findAndRemove(
          cssLinks,
          preloadLink.props.href,
          'stylesheet'
        )
        cssLinksSorted.push(preloadLink, stylesheetLink)
      }
    })
    return cssLinksSorted
  }
}
