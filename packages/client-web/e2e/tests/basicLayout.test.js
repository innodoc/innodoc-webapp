...
describe('Basic layout', () => {
  ...
  it('should have a header', async () => {
    ...
    await browser.assert.exists(
      '[class*=header___] [class*=headerLogoWrapper___]'
    )
    ...
    await browser.assert.exists(
      '[class*=header___] ul[class*=nav___] a[title="Download PDF"]'
    )
    await browser.assert.exists(
  ...
  it('should have a footer', async () => {
    ...
    const segments = await browser.queryAll(
      '[class*=footer___] [class*=footerSegment___]'
    )
    expect(segments).toHaveLength(3)

    const heading1 = await segments[0].query('h4')
    expect(await browser.evaluate((el) => el.textContent, heading1)).toBe(
      'innoDoc'
    )
